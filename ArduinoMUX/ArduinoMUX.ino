...
const int RCout1_pin = 5;
const int RCout2_pin = 6;
...
void setup() {
    ...
    RCout1.attach(RCout1_pin, 500, 2500);
    RCout2.attach(RCout2_pin, 500, 2500);
...
void loop() {  
    ...
    if(RC_avail() || now - rc_update > 25){   // if RC data is available or 25ms has passed since last update (adjust to > frame rate of receiver)
      ...
      if (RC_in[2] > 1500) 
        {
        RCout1.writeMicroseconds(servo1_uS);   // write the pulsewidth to the servo.
        RCout2.writeMicroseconds(servo2_uS);   // write the pulsewidth to the servo.
        }
      else
        {
        ...
        servo2_uS = calc_uS(RC_in[1],2);      // apply the servo rates, direction and sub_trim for servo 1, and convert to a RC pulsewidth (microseconds, uS)
